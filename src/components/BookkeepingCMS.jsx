...
import { db, associateCustomerToBuild, COMPONENT_LABELS } from '../db/database';
...
export default function BookkeepingCMS() {
  ...
  const getReconciliationData = () => {
    ...
    const grossMargin = totalInvoiceRevenue - totalPartsCost;
    const marginPercent = totalPartsCost > 0 ? (grossMargin / totalInvoiceRevenue) * 100 : 0;
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.25rem' }}>
              <div className="stat-chip" style={{ padding: '1.25rem', textAlign: 'left', background: 'var(--bg-card)' }}>
                ...
                <span className="stat-val" style={{ color: 'var(--text-main)', fontSize: '1.8rem' }}>
                  ${recon.metrics.totalPartsCost.toFixed(2)}
                </span>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>From linked builds</span>
              </div>

              <div className="stat-chip" style={{ padding: '1.25rem', textAlign: 'left', background: 'var(--bg-card)' }}>
                <span className="stat-lbl">Invoiced Revenue</span>
                <span className="stat-val" style={{ color: 'var(--primary)', fontSize: '1.8rem' }}>
                  ${recon.metrics.totalInvoiceRevenue.toFixed(2)}
                </span>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>From customer invoices</span>
              </div>

              <div className="stat-chip" style={{ padding: '1.25rem', textAlign: 'left', background: 'var(--bg-card)' }}>
                <span className="stat-lbl">Gross Profit Margin</span>
                <span className="stat-val" style={{ color: recon.metrics.grossMargin >= 0 ? 'var(--accent)' : 'var(--danger)', fontSize: '1.8rem' }}>
                  ${recon.metrics.grossMargin.toFixed(2)}
                </span>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                  {recon.metrics.marginPercent.toFixed(1)}% Markup margin
                </span>
              </div>
              ...
              <div className="stat-chip" style={{ padding: '1.25rem', textAlign: 'left', background: recon.metrics.unbilledLeakage > 0 ? 'rgba(239, 68, 68, 0.1)' : 'var(--bg-card)', border: recon.metrics.unbilledLeakage > 0 ? '1px solid var(--danger)' : '1px solid var(--border)' }}>
                ...
                <span className="stat-val" style={{ color: recon.metrics.unbilledLeakage > 0 ? 'var(--danger)' : 'var(--text-muted)', fontSize: '1.8rem' }}>
                ...
                {recon.billedItems.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    ...
                    {recon.unmatchedInvoiceItems.map(item => (
                      <div key={item.id} className="order-row" style={{ borderLeft: '4px solid var(--primary)', padding: '0.75rem' }}>
                        <div className="order-row-main">
                          ...
                          <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.35rem', fontSize: '0.78rem' }}>
                            <span style={{ color: 'var(--primary)', fontWeight: 'bold' }}>Amount Billed: ${item.total.toFixed(2)}</span>
                          </div>
                        </div>
                        <span className="status-badge status-received" style={{ fontSize: '0.7rem', color: 'var(--primary)', background: 'rgba(59, 130, 246, 0.15)' }}>
                          Labor/Stock
                        </span>
                      </div>
                    ))}
