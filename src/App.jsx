...
import BookkeepingCMS from './components/BookkeepingCMS';
...
export default function App() {
  ...
  const [view, setView] = useState('dashboard'); // 'dashboard', 'build', 'customers', 'service-board', 'invoices', 'bookkeeping'
        ...
        <nav className="header-nav" style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          {view === 'build' ? (
            ...
            <>
              ...
              <button
                className={`btn nav-tab${view === 'bookkeeping' ? ' nav-tab-active' : ''}`}
                onClick={() => { setView('bookkeeping'); setBuildId(null); }}
              >📊 Bookkeeping</button>
            </>
          )}
        </nav>
